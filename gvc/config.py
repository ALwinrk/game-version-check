...
# ── 慢速源（StealthySession）───────────────────────────────
# 注意：环境变量名是 GVC_DISABLE_STEALTH（设置为 "1" 则禁用），
# Python 变量 ENABLE_STEALTH_SOURCES 是取反后的结果（True = 启用）。
...
JS_RENDER_TIMEOUT: float = _env_float("GVC_JS_RENDER_TIMEOUT", 20.0)  # JS 渲染源超时 (无 CF)
