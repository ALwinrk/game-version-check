...
from gvc.config import REQUEST_TIMEOUT, MAX_RETRIES, RETRY_BACKOFF, STEALTH_TIMEOUT, JS_RENDER_TIMEOUT, get_proxies
...
def http_get(url: str) -> tuple[int, str]:
    ...
    try:
        ...
        logger.warning("Fetcher network error for %s: %s", url[:60], e)
        return 0, f"{type(e).__name__}: {e!s}"[:80]
    except Exception as e:
        logger.exception("Fetcher unexpected error for %s: %s", url[:60], e)
        return 0, f"{type(e).__name__}: {e!s}"[:80]
...
def _stealth_fetch_worker(
    url: str,
    proxy_config: dict | None,
    chrome_exe: str | None,
    result_queue: queue.Queue,
) -> None:
    """在独立线程中执行 StealthySession 浏览器请求."""
    try:
        with StealthySession(
            ...
            timeout=int((STEALTH_TIMEOUT + 15) * 1000),
            google_search=False,
            proxy=proxy_config,
            ...
            if resp.status == 200 and len(resp.html_content) > 500:
                if is_cloudflare_block(resp.html_content):
                    result_queue.put((0, f"Cloudflare bypass failed: {urlparse(url).hostname}"))
                else:
...
def stealth_get(url: str, *, use_proxy: bool = False) -> tuple[int, str]:
    ...
    内置硬超时保护，防止 Cloudflare turnstile 无限等待.
    ...
    with _stealthy_lock:
        ...
        # 硬超时 = STEALTH_TIMEOUT + 30s 缓冲 (CF 挑战最长等待)
        hard_timeout = STEALTH_TIMEOUT + 30.0
        result_queue: queue.Queue = queue.Queue()
        worker = threading.Thread(
            target=_stealth_fetch_worker,
            args=(url, proxy_config, chrome_exe, result_queue),
            daemon=True,
        )
        worker.start()
        ...
        if worker.is_alive():
            logger.warning(
                "StealthySession timeout after %.0fs for %s (CF challenge stuck)",
                hard_timeout, url[:60],
            )
            return 0, f"StealthySession timeout after {hard_timeout:.0f}s (CF challenge)"

        try:
            return result_queue.get_nowait()
        except queue.Empty:
            return 0, "StealthySession error: no result produced"


# ── JS 渲染后端: StealthySession (纯渲染, 无 CF 等待) ─────────


def js_render_get(url: str) -> tuple[int, str]:
    """JS 渲染 GET — 用于需要 JavaScript 但无 Cloudflare 的站点.

    通过 Chromium 无头浏览器加载页面并执行 JS,
    但跳过 CF challenge 解决 (solve_cloudflare=False),
    比 stealth_get 快得多 (~3-8s/页).
    ...
    with _stealthy_lock:
        ...
        hard_timeout = JS_RENDER_TIMEOUT + 15.0
        result_queue: queue.Queue = queue.Queue()
        ...
        def _worker():
            try:
                with StealthySession(
                    headless=True,
                    solve_cloudflare=False,   # 跳过 CF 等待
                    block_ads=True,
                    disable_resources=True,
                    timeout=int(JS_RENDER_TIMEOUT * 1000),
                    google_search=False,
                    proxy=proxy_config,
                    executable_path=chrome_exe,
                ) as s:
        ...
        if worker.is_alive():
            logger.warning(
                "js_render timeout after %.0fs for %s",
                hard_timeout, url[:60],
            )
