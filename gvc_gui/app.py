...
class MainApplication(ctk.CTk):
    ...
    def __init__(self) -> None:
        ...
        # 首次运行时预填默认代理（用户可在设置中修改或清空）
        for key, val in [("http_proxy", "http://127.0.0.1:7897"),
                         ("https_proxy", "http://127.0.0.1:7897")]:
            self._settings.setdefault(key, val)
    ...
    def _on_open_file(self) -> None:
        ...
        if path:
            ...
            try:
                ...
                import logging
                logging.getLogger("gvc").exception("加载 Excel 失败: %s", path)
                self._status_label.configure(text=f" 加载失败: {os.path.basename(path)} — 文件可能已损坏或格式不正确")
    ...
    def _apply_runtime_settings(self) -> None:
        ...
        self._apply_env_settings(self._settings, defaults=True)
    ...
    def _apply_env_settings(self, settings: dict, *, defaults: bool = False) -> None:
        """将代理/超时/并发/下载等运行时设置写入环境变量."""
        for key, custom_env in [("http_proxy", "GVC_HTTP_PROXY"), ("https_proxy", "GVC_HTTPS_PROXY")]:
            ...
            if val:
                ...
                for v in (custom_env, std_env):
                    if v in os.environ:
                        del os.environ[v]
        # 仅在 defaults 模式下跳过未设置值的环境变量写入
        for setting_key, env_name, default in [
            ("request_timeout", "GVC_REQUEST_TIMEOUT", 10),
            ("max_game_workers", "GVC_MAX_GAME_WORKERS", 3),
        ]:
            val = settings.get(setting_key, default if defaults else None)
            if val is not None:
                os.environ[env_name] = str(val)
        # 下载管理器 & 目录 — 仅在显式设置时写入
        for setting_key, env_name in [
            ("download_manager", "GVC_DOWNLOAD_MANAGER"),
            ("download_dir", "GVC_DOWNLOAD_DIR"),
        ]:
